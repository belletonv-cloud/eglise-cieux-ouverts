<template>
  ...
  <BlockYoutube
    ...
    :data-admin="isAdmin || undefined"
  />
  <div v-else style="display:none" data-block-renderer-fallback="true">{{ btype }}</div>
</template>
...
function clean(value, path = '') {
  ...
  if (Array.isArray(value)) return value.map((v, i) => clean(v, path ? `${path}[${i}]` : `[${i}]`))
  const res = {}
  for (const [k, v] of Object.entries(value)) res[k] = clean(v, path ? `${path}.${k}` : k)
  return res
}
